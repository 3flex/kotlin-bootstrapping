...
build/kotlin-1.0.0-beta-4091: build/kotlin-1.0.0-beta-3070
	$(call kcheckout,1.0.0-beta-4091)
	cd kotlin \
	    && rm -Rf ideaSDK dependencies \
	    && mkdir -p ideaSDK/lib ideaSDK/core ideaSDK/core-analysis ideaSDK/jps \
	    && mkdir -p dependencies/ant-1.8/lib dependencies/annotations \
	    ...
	    && cp /usr/share/java/native-platform.jar dependencies/native-platform-uberjar.jar \
	    && cp /usr/share/ant/lib/ant.jar          dependencies/ant-1.8/lib/ \
	    && ANT_OPTS=-noverify ant -Dshrink=false -Dgenerate.javadoc=false -Dbootstrap.build.no.tests=true -Dbootstrap.compiler.home=../build/kotlin-1.0.0-beta-3070 \
	    && mv dist/kotlinc ../build/kotlin-1.0.0-beta-4091

build/kotlin-1.0.0-beta-5010: build/kotlin-1.0.0-beta-4091
	$(call kcheckout,1.0.0-beta-5010)
	cd kotlin \
	    && rm -Rf ideaSDK dependencies \
	    && mkdir -p ideaSDK/lib ideaSDK/core ideaSDK/core-analysis ideaSDK/jps \
	    && mkdir -p dependencies/ant-1.8/lib dependencies/annotations \
	    ...
	    && cp /usr/share/java/native-platform.jar dependencies/native-platform-uberjar.jar \
	    && cp /usr/share/ant/lib/ant.jar          dependencies/ant-1.8/lib/ \
	    && ANT_OPTS=-noverify ant -Dshrink=false -Dgenerate.javadoc=false -Dbootstrap.build.no.tests=true -Dbootstrap.compiler.home=../build/kotlin-1.0.0-beta-4091 \
	    && mv dist/kotlinc ../build/kotlin-1.0.0-beta-5010

build/kotlin-1.0.0-beta-5604: build/kotlin-1.0.0-beta-5010
	$(call kcheckout,1.0.0-beta-5604)
	cd kotlin \
	    && rm -Rf ideaSDK dependencies \
	    && mkdir -p ideaSDK/lib ideaSDK/core ideaSDK/core-analysis ideaSDK/jps \
	    && mkdir -p dependencies/ant-1.8/lib dependencies/annotations \
	    ...
	    && cp /usr/share/java/native-platform.jar dependencies/native-platform-uberjar.jar \
	    && cp /usr/share/ant/lib/ant.jar          dependencies/ant-1.8/lib/ \
	    && ANT_OPTS=-noverify ant -Dshrink=false -Dgenerate.javadoc=false -Dbootstrap.build.no.tests=true -Dbootstrap.compiler.home=../build/kotlin-1.0.0-beta-5010 \
	    && mv dist/kotlinc ../build/kotlin-1.0.0-beta-5604

build/kotlin-1.0.0-dev-162: build/kotlin-1.0.0-beta-5604
	$(call kcheckout,1.0.0-dev-162)
	cd kotlin \
	    && rm -Rf ideaSDK dependencies \
	    && mkdir -p ideaSDK/lib ideaSDK/core ideaSDK/core-analysis ideaSDK/jps \
	    && mkdir -p dependencies/ant-1.8/lib dependencies/annotations \
	    ...
	    && cp /usr/share/java/native-platform.jar dependencies/native-platform-uberjar.jar \
	    && cp /usr/share/ant/lib/ant.jar          dependencies/ant-1.8/lib/ \
	    && ANT_OPTS=-noverify ant -Dshrink=false -Dgenerate.javadoc=false -Dbootstrap.build.no.tests=true -Dbootstrap.compiler.home=../build/kotlin-1.0.0-beta-5604 \
	    && mv dist/kotlinc ../build/kotlin-1.0.0-dev-162

build/kotlin-1.0.0: build/kotlin-1.0.0-dev-162
	$(call kcheckout,1.0.0)
	cd kotlin \
	    && rm -Rf ideaSDK dependencies \
	    && mkdir -p ideaSDK/lib ideaSDK/core ideaSDK/core-analysis ideaSDK/jps \
	    && mkdir -p dependencies/ant-1.8/lib dependencies/annotations \
	    ...
	    && cp /usr/share/java/native-platform.jar dependencies/native-platform-uberjar.jar \
	    && cp /usr/share/ant/lib/ant.jar          dependencies/ant-1.8/lib/ \
	    && ANT_OPTS=-noverify ant -Dshrink=false -Dgenerate.javadoc=false -Dbootstrap.build.no.tests=true -Dbootstrap.compiler.home=../build/kotlin-1.0.0-dev-162 \
	    && mv dist/kotlinc ../build/kotlin-1.0.0

build/kotlin-1.1.0: build/kotlin-1.0.0
	$(call kcheckout,1.1.0)
	cd kotlin \
	    && rm -Rf ideaSDK dependencies \
	    && mkdir -p ideaSDK/lib ideaSDK/core ideaSDK/core-analysis ideaSDK/jps \
	    && mkdir -p dependencies/ant-1.8/lib dependencies/annotations \
	    ...
	    && cp /usr/share/java/native-platform.jar dependencies/native-platform-uberjar.jar \
	    && cp /usr/share/ant/lib/ant.jar          dependencies/ant-1.8/lib/ \
	    && ANT_OPTS=-noverify ant -Dshrink=false -Dgenerate.javadoc=false -Dbootstrap.build.no.tests=true -Dbootstrap.compiler.home=../build/kotlin-1.0.0 \
	    && mv dist/kotlinc ../build/kotlin-1.1.0
